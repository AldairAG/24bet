...
public class ApuestaService {
    ...
    public ParlayResponse crearParlay(ParlayRequest parlayRequest, Usuario usuario) {
        ...
        for (ParlayRequest.ApuestaParlay apuestaReq : parlayRequest.getApuestas()) {
            ...
            valor.setNumeroApuestas((valor.getNumeroApuestas() != null ? valor.getNumeroApuestas() : 0) + 1);
    ...
    public List<ParlayResponse> obtenerParlaysActivos(Long usuarioId) {
        List<Parlay> parlaysActivos = parlayRepository.findByUsuarioIdAndEstado(usuarioId, Parlay.EstadoParlay.ACTIVO);
    ...
    private ParlayResponse convertirParlayAResponse(Parlay parlay, List<Apuesta> apuestas) {
        List<ParlayResponse.ApuestaParlayResponse> apuestasResponse = apuestas != null ? 
                apuestas.stream()
                        .map(apuesta -> ParlayResponse.ApuestaParlayResponse.builder()
                                ...
                                .nombreEvento(apuesta.getEventoDeportivo() != null ? 
                                        apuesta.getEventoDeportivo().getEquipoLocal() + " vs " + apuesta.getEventoDeportivo().getEquipoVisitante() : 
